      ...
      <form @submit.prevent="handleSignup" class="mt-8 space-y-6">
        <div class="space-y-4 rounded-md">
          ...
          <div>
            ...
            <input
              ...
              placeholder="••••••••"
            />
            <p v-if="form.passwordConfirm && form.password !== form.passwordConfirm" class="mt-1 text-xs text-red-500">
              비밀번호가 일치하지 않습니다
            </p>
          </div>
        ...
        <div class="space-y-3">
          <div class="flex items-center">
            ...
            />
            <label for="agreeAll" class="ml-2 block text-sm font-medium text-gray-900 cursor-pointer">
              전체 동의
            </label>
          </div>
          ...
          <div class="ml-6 space-y-2">
            ...
            <div class="flex items-center">
              ...
              <label for="agreePrivacy" class="ml-2 block text-sm text-gray-700 cursor-pointer">
                <span class="text-red-500">*</span> 개인정보 수집 및 이용 동의
              </label>
            </div>
            ...
            <div class="flex items-center">
              <input
                ...
                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded cursor-pointer"
              />
              <label for="agreeMarketing" class="ml-2 block text-sm text-gray-700 cursor-pointer">
                마케팅 정보 수신 동의 (선택)
              </label>
            </div>
          </div>
...
export default {
  ...
  watch: {
    ...
    'form.agreeMarketing': 'updateAgreeAll'
  },
  methods: {
    toggleAllAgreements () {
      this.form.agreeTerms = this.agreeAll
      this.form.agreePrivacy = this.agreeAll
      this.form.agreeMarketing = this.agreeAll
    },
    updateAgreeAll () {
      this.agreeAll = this.form.agreeTerms && this.form.agreePrivacy && this.form.agreeMarketing
    },
    async handleSignup () {
      this.isLoading = true
      this.errorMessage = ''
      ...
      try {
        ...
        this.errorMessage = error.message || '회원가입 중 오류가 발생했습니다.'
